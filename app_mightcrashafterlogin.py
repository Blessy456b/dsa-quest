    ...
    for step_key, step_data in STRIVER_SHEET.items():
        ...
        with c2:
            if st.button(f"Start {step_key}", key=f"start_{step_key}"):
                ...
                # stop current run so the change takes effect on the next rerun
                st.rerun()
                return
...
def render_problem_browser():
    ...
    # choose index based on session state if available
    selected_index = 0
...
def render_problem_detail(problem: dict, xp_reward: int):
    ...
    # Provide Generate and Regenerate buttons that fetch content and store it in session_state cache.
    ...
    with col_gen:
        if st.button("✨ Generate Story & Analogy", key=f"generate_{problem['id']}"):
            with st.spinner("🪄 Creating magical learning content with AI..."):
                ...
                # After generation, keep user on same page and show the results (no st.stop)
    with col_regen:
    ...
    if content_key in st.session_state["content_cache"]:
        ...
        # Learning tip shown as info
        st.info(f"**💡 Learning Tip:** {content.get('learning_tip', 'No tip available.')}")

        # --- Debug / Raw LLM Response (development only) ---
        st.markdown("---")
        st.markdown("### 🔎 Debug: Raw LLM response (dev-only)")
        # These keys are written by langraph_agent._invoke_llm_for when Streamlit is available.
