...
from games import render_games  # ✅ NEW: games page
    ...
    with tab2:
        ...
        if st.button("Create account"):
            ...
            if error:
                ...
                # 🎉 Celebration + clear message
                st.balloons()
                st.success("🎉 Account created! Check your inbox to verify.")
                st.info("Return and sign in after verification.")
...
def render_home():
    ...
    for step_key, step_data in STRIVER_SHEET.items():
        ...
        # (Optional) Start button per step – can be added if needed
...
def render_problem_detail(problem: dict, xp_reward: int):
    ...
    if st.button("Save My Solution 💾", key=f"save_{problem['id']}"):
     ...
     # -------------------------------
    # ✅ NEW: AI-POWERED SOLUTION EVALUATOR
    # -------------------------------
    from llm_evaluator import evaluate_solution

    st.markdown("### 🤖 AI Evaluation")

    if st.button("Check My Solution 🤖", key=f"eval_{problem['id']}"):
        if not _user_code.strip():
            st.warning("Write your code before checking!")
        else:
            with st.spinner("AI is reviewing your solution..."):
                result = evaluate_solution(
                    problem_title=problem["title"],
                    problem_description=", ".join(problem.get("concepts", [])),
                    user_code=_user_code,
                    language=language
                )

            if "error" in result:
                st.error("⚠️ Evaluation Error: " + result["error"])
            else:
                st.success("✅ Evaluation Complete")

                st.subheader("Correctness")
                st.write(result["correctness"])

                st.subheader("Logic Review")
                st.write(result["logic_review"])

                st.subheader("Edge Cases")
                st.write(result["edge_cases"])

                st.subheader("Time Complexity Analysis")
                st.write(result["time_complexity"])
