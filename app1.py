...
# ---------- Sidebar ----------
def render_sidebar():
    """Render sidebar with navigation and user stats."""
    ...
    with st.sidebar:
        ...
        # Safety: ensure tracker has expected attributes
        try:
            current_badge = tracker.get_current_badge()
        except Exception:
            current_badge = {"name": "🌱 DSA Rookie"}

        st.markdown("### Your Progress")
        st.markdown(f"**{current_badge.get('name', '🌱 DSA Rookie')}**")
        st.markdown(f"**XP:** {getattr(tracker, 'total_xp', 0)} 💎")
        st.markdown(f"**Streak:** {getattr(tracker, 'current_streak', 0)} 🔥")
        st.markdown(f"**Problems Solved:** {len(getattr(tracker, 'completed_problems', []))}/{TOTAL_PROBLEMS}")

        progress = tracker.get_progress_percentage() if hasattr(tracker, "get_progress_percentage") else 0.0
        ...
        with st.expander("⚙️ Settings"):
            ...
            st.markdown("Add your API key to unlock AI-powered stories and analogies!")
            try:
...
def render_problem_browser():
    ...
    selected_step = st.selectbox("Select Topic", step_options, index=0 if not st.session_state.get("current_step") else
                                 next((i for i, s in enumerate(step_options) if s.startswith(st.session_state["current_step"])), 0))
...
def render_problem_detail(problem: dict, xp_reward: int):
    ...
    with h3:
        if is_completed:
            ...
            if st.button("Mark Complete 🎉", key=f"complete_{problem['id']}"):
                is_new, new_badges = tracker.mark_problem_complete(problem["id"], xp_reward)
                if is_new:
    ...
    # ensure content_cache exists
    if "content_cache" not in st.session_state:
        st.session_state.content_cache = {}

    # Generate or regenerate
    if content_key not in st.session_state["content_cache"]:
        if st.button("✨ Generate Story & Analogy", key=f"generate_{problem['id']}"):
            with st.spinner("🪄 Creating magical learning content with AI..."):
                try:
                    content = generate_learning_content(problem["title"], problem.get("concepts", []), problem.get("difficulty", "Medium"))
                except Exception as e:
                    content = {"story": f"⚠️ Error: {e}", "analogy": "Unavailable", "learning_tip": "Unavailable", "has_error": True}
    ...
    if content_key in st.session_state["content_cache"]:
        ...
        # show warnings if generation had an error
        if content.get("has_error"):
            st.warning("⚠️ There was an issue generating AI content. Showing fallback/partial content.")
        ...
        st.info(f"**💡 Learning Tip:** {content.get('learning_tip', 'No tip available.')}")
    ...
    if st.button("Save My Solution 💾", key=f"save_{problem['id']}"):
        # For now just show success — you can extend to persist solutions to user_data/
...
def render_profile():
    ...
    step_xp_data = []
    for step_key, step_data in STRIVER_SHEET.items():
        completed_in_step = sum(1 for p in step_data.get("problems", []) if p["id"] in tracker.completed_problems)
...
def render_achievements():
    ...
    if next_badge:
        ...
        progress_pct = (tracker.total_xp / next_badge["xp_required"]) * 100 if next_badge["xp_required"] else 100
