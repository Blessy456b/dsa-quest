...
# ---------- Auth UI ----------
def show_auth_ui():
    """Account section in sidebar: Sign in / Sign up / Sign out."""
    ...
    if user:
        ...
        with tab2:
            ...
            if st.button("Create account"):
                res = sign_up(email, password, username=username)
                if getattr(res, "error", None):
...
def get_current_badge_from_tracker(tracker):
    ...
    # fallback based on XP thresholds (optional)
    return {"name": "🌱 DSA Rookie"}
...
def main():
    ...
    show_auth_ui()  # account UI at top of sidebar
    page = render_sidebar()

    user = get_current_user()
    if not user:
        st.info("Please sign in or sign up from the sidebar to start your DSA Quest.")
        return
